...
void RenderManager::executeRotateScript(string script_file_name, string script_function_name, string object_name, int degrees)
{
   ...
   selected_node->setOrientation(w*5, x*5, y*5, z*5);
...
DebugDrawer* RenderManager::createOgreBulletDebugDrawer()
{

   Ogre::ManualObject* manual_object =  scene_manager->createManualObject("PhysicsLines");
...
void RenderManager::drawLine(DebugDrawer* db, float* from_value, float* to_value, float* color_value, int* count)
{
   ...
   manual_object->position(from); 
   manual_object->colour(c);
   manual_object->position(to);
   manual_object->colour(c);
   manual_object->end();
}
void RenderManager::clearLines(DebugDrawer* db)
{
   Ogre::ManualObject* manual_object = db->manual_object;
   manual_object->clear();
}
void RenderManager::destroyDebugDrawer(DebugDrawer* db)
{
   // needs revision
   // free(db->manual_object_node);
   // free(db->manual_object);
   // delete db;
}
